...
const login = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className='flex, flex-col gap-4 m-auto items-start p-8 py-12 w-80 sm:w-[352px] text-gray-500 rounded-lg shadow-xl border border-gray-200 bg-white'>
      ...
      <div className='w-full'>
        <p>Email</p>
          <input onChange={(e) => setName(e.target.value)} value={email} placeholder='type here' className='border border-gray-200 rounded w-full p-2 mt-1 outline-purple-700' type='text' required/>
      </div>
      <div className='w-full'>
        <p>Password</p>
          <input onChange={(e) => setName(e.target.value)} value={password} placeholder='type here' className='border border-gray-200 rounded w-full p-2 mt-1 outline-purple-700' type='text' required/>
      </div>
      {state === 'register' ? (
        <p>
        ...
        <p>
          Create an account? <span onClick={() => setState("login")}
          className='text-purple-700 cursor-pointer'>Click here</span>
        </p>
      )}
